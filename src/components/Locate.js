...
const Locate = () => {
  ...
  const locationPagesRef = useRef(null);

  useEffect(() => {
    if (location.hash === '#location-pages' && locationPagesRef.current) {
      locationPagesRef.current.scrollIntoView({ behavior: 'auto', block: 'start' });
    }
  }, [location]);
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ...
      { threshold: 1 } // Adjust this value to control when the effect triggers0
    );
  ...
  return (
    <>
      ...
      <div id="location-pages" ref={locationPagesRef} className="location">
